...
public class ScalerModuleDemo {
    ...
    public static void main(String[] args) {
        try {
            ...
            ChannelContainer cc = new ChannelContainer(file, 1, decodedMetadata.get(1));
            ...
            ScalerModule sm = new ScalerModule(container.getMetadata());
            FloatProcessor fp = new FloatProcessor(1024, 1024);
            for (int i = 0; i < 1024; i++) {
                for (int j = 0; j < 1024; j++) {
                    long pixelValue = imageProcessor.get(i, j);
                    double scaledPixelValueInMeter = sm.scaleToObtainPhysicalValue(pixelValue) * Math.pow(10, 9);
